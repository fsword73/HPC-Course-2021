...
__global__ void gauss_filter3x3(int* A, int* B,  int h, int w )
{
    ...
    for(int i=0; i< 3; i++){
        for(int j=0; j< 3; j++){
            sum +=data[j+i *3] * gauss_filter[j+i*3];
        }//forj
    }//fori
    B[x + y *w ] =(int)sum;
}

__global__ void gauss_filter3x3_tile4x4(int* A, int* B,  int h, int w ){
        ...
        int sidx = s*4 + t;
           ...
           for(int i=0; i< 3; i++){
                for(int j=0; j< 3; j++){
                    int xx = j+t;
                    int yy = i+s;
                    sum[sidx] +=data[ xx+yy * 6] * gauss_filter[j+i*3];
                }//forj
            }//fori
        }//fort
    for(int s=0; s<4; s++){
...
int main() {
   ...
   // Size of vectors
    ...
    gauss_filter3x3<<<gridSize, blockSize>>>(d_a,d_b,1024,1024);
    ...
    cudaEventRecord( start,0);
    dim3 gridsize2(8,8);
    gauss_filter3x3_tile4x4<<<gridSize, gridsize2>>>(d_a,d_b,1024,1024);
    cudaEventRecord( stop,0);
    ...
    save_ppm(h_b, 1024,1024,"chess_gauss.ppm");
